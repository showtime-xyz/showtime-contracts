...
contract("ShowtimeMT", async (accounts) => {
  ...
  xdescribe("Transfer Tests", async () => {
    beforeEach(async () => {
      // Deploy MultiToken
      mt = await MT.new();
      // Issue MT
  ...
  describe("Royalty tests", function () {
    ...
    it("mints token with royalty", async function () {
      ...
      const royaltyPercent = 250; // 2.5% royalty
      await mt.issueToken(
        recipient,
        amount,
        hash,
      ...
      assert.equal(royaltyAmount.toString(), "250".padEnd(17, "0")); // 2.5% of 1 ether
    });
    it("mints in batch with royalty", async function () {
      ...
      for (i = 0; i < iterations; i++) {
        const balance = await mt.balanceOf(accounts[1], tokenIds[i]);
        assert.equal(balance, amounts[i]);
        const { receiver, royaltyAmount } = await mt.royaltyInfo(
          tokenIds[i],
          web3.utils.toWei("1")
        );
        ...
        assert.equal(
          ...
          (50 * (i + 1)).toString().padEnd(i ? 17 : 16, "0")
        );
        const URI = await mt.uri(tokenIds[i]);
        assert.equal(URI, "https://gateway.pinata.cloud/ipfs/some-hash-" + i);
      }
