...
contract("ERC1155 Sale Contract Tests", (accounts) => {
    ...
    beforeEach(async () => {
        ...
        // approvals
        sale = await ERC1155Exchange.new(mt.address, [token.address]);
    ...
    it("doesn't allow to deploy with incorrect constructor arguments", async () => {
        await truffleAsserts.reverts(ERC1155Exchange.new(alice, [token.address]), "must be contract address");
        await truffleAsserts.reverts(
            ERC1155Exchange.new(mt.address, [ZERO_ADDRESS]),
